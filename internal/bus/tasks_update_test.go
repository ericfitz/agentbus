...
func TestOwnerGuard(t *testing.T) {
	...
	wantCode(t, err, "conflict")
	parent := int64(0)
	_, err = other.TaskUpdate("Pat", TaskPatch{ID: tk.ID, Parent: &parent})
	wantCode(t, err, "conflict")
	_, err = other.TaskUpdate("Pat", TaskPatch{ID: tk.ID, Delete: true})
	wantCode(t, err, "conflict")

	// Sam (the owner) can do each.
...
func TestStatusTransitions(t *testing.T) {
	...
	tk = fresh()
	if _, err := b.TaskUpdate("Sam", TaskPatch{ID: tk.ID, Status: &completed, Owner: &owner}); err != nil {
		t.Fatal(err)
	}
	_, err = b.TaskUpdate("Sam", TaskPatch{ID: tk.ID, Status: &st})
	wantCode(t, err, "validation")
...
func TestUnknownOwnerIsNotFound(t *testing.T) {
	...
	got, _ := b.TaskGet("Sam", tk.ID)
	if got.Revision != 1 {
		t.Fatalf("revision changed after refusal: %+v", got)
	}
	pat := "Pat"
...
func TestParentRules(t *testing.T) {
	...
	bParent := bb.ID
	if _, err := b.TaskUpdate("Sam", TaskPatch{ID: a.ID, Parent: &bParent}); err != nil {
		t.Fatal(err)
	}
	aParent := a.ID
